...
const webhookRegex1 = /if \(settings\.notifType === 'telegram' && settings\.telegramWebhook\) \{[\s\S]*?\}\s*\}\s*\}/;
ordersCode = ordersCode.replace(webhookRegex1, '');


// Replace feedback telegram logic
const feedbackRegex = /if \(settings\.notifType === 'telegram' && settings\.telegramWebhook\) \{[\s\S]*?body: JSON\.stringify\(\{\s*event: 'feedback',[\s\S]*?\}\)[\s\S]*?\}\s*\}\s*\}/;
